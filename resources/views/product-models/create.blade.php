<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-4">
            <div class="min-w-0 flex-1">
                <h2 class="font-semibold text-lg sm:text-xl text-gray-900 truncate">Créer un modèle de produit</h2>
                <p class="text-xs sm:text-sm text-gray-500 mt-0.5 sm:mt-1">Ajoutez un nouveau modèle à votre catalogue</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm">
        <form method="POST" action="{{ route('product-models.store') }}" class="p-4 sm:p-6 lg:p-8">
            @csrf
